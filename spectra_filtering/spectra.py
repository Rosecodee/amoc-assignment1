...
def raw_periodogram(
    ...
    win_norm = np.sum(win ** 2)  # normalisation 
 
    xw = x * win
    ft = np.fft.rfft(xw)
 
    # Two-sided power, then fold to one-sided
    psd = (np.abs(ft) ** 2) / win_norm * dt_days
    # Double all bins except DC and Nyquist
    psd[1:-1] *= 2
 
    freq = frequency_axis(N, dt_days)
    return freq, psd
...
def welch_psd(
    ...
    x = np.asarray(x, dtype="float64")
    fs = 1.0 / dt_days                       # sampling freq
    noverlap = int(segment_length * overlap)
 
    freq_hz, psd_hz = signal.welch(
        x,
        fs=fs,
        window=window,
